...
export async function placeOrder(input: any) {
  ...
  if (
    input.side === "sell"
      ? parseFloat(accountAssetData.quantity) < input.quantity
      : parseFloat(accountAssetData.quantity) < input.price
  )
    throw new Error("Insufficient amount for this order");

  const ordersData = await selectOrders(
    input.accountId,
    input.marketId,
    input.side,
  );
  ...
  if (
    input.side === "sell"
      ? parseFloat(accountAssetData.quantity) < input.quantity + totalAmout
      : parseFloat(accountAssetData.quantity) < input.price + totalAmout
  )
    throw new Error("Insufficient amount for this order");
